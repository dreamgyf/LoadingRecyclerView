...
public class SampleActivity extends AppCompatActivity {
	...
	public static class LayoutManagerType {
		public final static int LINEAR_LAYOUT_MANAGER = 0;
		public final static int GRID_LAYOUT_MANAGER = 1;
		public final static int STAGGERED_GRID_LAYOUT_MANAGER = 2;
	}
	...
	public static Intent createIntent(Context context, int layoutManagerType, int orientationType) {
		Intent intent = new Intent(context, SampleActivity.class);
		intent.putExtra("LayoutManagerType", layoutManagerType);
	...
	private void parseIntent() {
		...
		int layoutManagerType = intent.getIntExtra("LayoutManagerType", LayoutManagerType.LINEAR_LAYOUT_MANAGER);
		int orientationType = intent.getIntExtra("OrientationType", OrientationType.VERTICAL);
		switch (layoutManagerType) {
			...
			case LayoutManagerType.GRID_LAYOUT_MANAGER: {
				mLoadingLayoutManager = new GridLayoutManager(this, 2);
				if (orientationType == OrientationType.HORIZONTAL) {
					((GridLayoutManager) mLoadingLayoutManager).setOrientation(RecyclerView.HORIZONTAL);
				}
			}
			break;
			case LayoutManagerType.STAGGERED_GRID_LAYOUT_MANAGER: {
				mLoadingLayoutManager = new StaggeredGridLayoutManager(2
						, orientationType == OrientationType.HORIZONTAL ? StaggeredGridLayoutManager.HORIZONTAL : StaggeredGridLayoutManager.VERTICAL);
			}
			break;
		}
	}
